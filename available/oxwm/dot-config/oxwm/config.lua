...
local dmenu_font = "BlexMono Nerd Font Propo:size=12"
...
local browser = "brave"
local dmenu_run = "dmenu_run -l 10" ..
    " -fn '" .. dmenu_font .. "'" ..
    " -nb '" .. colors.bg .. "'" ..
    " -nf '" .. colors.fg .. "'" ..
    " -sb '" .. colors.purple .. "'" ..
...
-- Launch Browser
oxwm.key.bind({ modkey }, "B", oxwm.spawn({ "sh", "-c", browser }))
-- Launch Dmenu
oxwm.key.bind({ modkey }, "D", oxwm.spawn({ "sh", "-c", dmenu_run }))
-- oxwm.key.bind({ modkey }, "D", oxwm.spawn({ "sh", "-c", "rofi -show drun" }))

-- screen rotation
-- oxwm.key.bind({ modkey }, "Left", oxwm.spawn({ "sh", "-c", "xrandr --output eDP1 --rotate right" }))
-- oxwm.key.bind({ modkey }, "Up", oxwm.spawn({ "sh", "-c", "xrandr --output eDP1 --rotate normal" }))
-- oxwm.key.bind({ modkey }, "Right", oxwm.spawn({ "sh", "-c", "xrandr --output eDP1 --rotate left" }))

-- Copy screenshot to clipboard
oxwm.key.bind({ modkey }, "S", oxwm.spawn({ "sh", "-c", "screenshot -f" }))
oxwm.key.bind({ modkey, "Shift" }, "S", oxwm.spawn({ "sh", "-c", "screenshot -s" }))

oxwm.key.bind({ modkey }, "Q", oxwm.client.kill())
-- Login screen
oxwm.key.bind({ modkey }, "L", oxwm.spawn({ "sh", "-c", "xsecurelock" }))
